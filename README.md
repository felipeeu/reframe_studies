...
* [`src/cljs/reframe_studies/`](src/cljs/reframe_studies/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/reframe_studies/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `reframe-studies` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_studies/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-studies.example
  (:require [reframe-studies.config :as config])
